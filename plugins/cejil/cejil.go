...
func init() {
	...
	w.OnHTML(`.brd_btm > div.txt-rel > h3 > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Title
	w.OnHTML(`article > div.box-content > h1`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Title = strings.TrimSpace(element.Text)
		})

	// 获取 PublicationTime
	w.OnHTML(`article > div.box-content > .date`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.PublicationTime = strings.TrimSpace(element.Text)
		})

	// 获取 Content
	w.OnHTML(`article > div.box-content`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Content = strings.TrimSpace(element.ChildText("p"))
		})

	// 获取 Tags
	w.OnHTML(`article > div.box-content > .tags`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Tags = append(ctx.Tags, strings.TrimSpace(element.Text))
		})

	// 获取 File
	w.OnHTML(`[class="box-widget download"] a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
