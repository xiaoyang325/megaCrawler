...
func init() {
	...
	// 访问下一页 Index
	w.OnHTML(`ul[class="pager pager-load-more"] > li[class="pager-next first last"] > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.Index)
		})

	// 访问 News 从 Index
	w.OnHTML(`.view-content > .item-list li .article-image > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.News)
		})

	// 获取 Title
	w.OnHTML(`body > div.container > div > div.section.section-width-sidebar.single-article-header > h2`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Title = strings.TrimSpace(element.Text)
		})

	// 获取 PublicationTime
	w.OnHTML(`body > div.container > div > div.section.section-width-sidebar.single-article-header > div > p`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			raw := element.Text
	...
	// 获取 Authors
	w.OnHTML(`body > div.container > div > div.section.section-width-sidebar.single-article-header > div > p > a  > span`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Authors = append(ctx.Authors, strings.TrimSpace(element.Text))
		})

	// 获取 Content
	w.OnHTML(`div[itemprop="articleBody"]`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
