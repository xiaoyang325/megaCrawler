...
func partNews(w *Crawler.WebsiteEngine) {
	...
	w.OnHTML(`div[class="btn btn-su-alert"] a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.Index)
		})

	// 从 Index 访问 Report
	w.OnHTML(".card-content h3 a",
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.Report)
		})

	// 获取 Title
	w.OnHTML("#story-head div h1",
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Content
	w.OnHTML("#story-content",
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Content = element.ChildText("p")
		})

	// 获取 Tags
	w.OnHTML(`div[class="btn btn-category"] a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
