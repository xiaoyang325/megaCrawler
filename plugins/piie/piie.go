...
func cut_out_names(name_str string) []string {
   reg, _ := regexp.Compile("(?U)[(].*[)]")
   out_and, _ := regexp.Compile("and")

   result := reg.ReplaceAllString(name_str, "")
   result2 := out_and.ReplaceAllString(result, ",")
...
func init() {
   w := megaCrawler.Register("Peterson Institute for International Economics", 
   ...
   w.OnHTML("a[title=\"Go to next page\"]",
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         to_url := strings.Split(ctx.Url, "?")[0] + element.Attr("href")
         w.Visit(to_url, megaCrawler.Index)
   })

   // 从Index中进入文章（情况一）
   w.OnHTML(".view__row>article>.teaser__body>.teaser__title>span>a", 
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
            to_url := "https://www.piie.com/" + element.Attr("href")
            if strings.Contains(to_url, "/blogs/") || strings.Contains(to_url, "/events/") {
   ...
   w.OnHTML(".hero-banner-event__speakers>div>.field__item>p",
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         ctx.Authors = append(ctx.Authors, cut_out_names(element.Text)...)
      })
   ...
   w.OnHTML(".download-button>a", 
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         file_url := "https://www.piie.com" + element.Attr("href")
