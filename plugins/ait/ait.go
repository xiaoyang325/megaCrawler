...
func init() {
	...
	// 获取 Title
	w.OnHTML(`div[class="internal-title-header medium"]`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Title = strings.TrimSpace(element.Text)
		})

	// 获取 Publication Time
	w.OnHTML(".main-content-wrapper",
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Authors
	w.OnHTML(`.authormeta > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Authors = append(ctx.Authors, strings.TrimSpace(element.Text))
		})

	// 获取 Content
	w.OnHTML(`div[class="paragraph alignwide container"] > div`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Content = element.Text
		})

	// 获取 Tags
	w.OnHTML(`.tagsmeta a[rel="tag"]`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Tags = append(ctx.Tags, element.Text)
		})

	// 获取 Location
	w.OnHTML(`.locationmeta > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
