...
func init() {
	w := Crawler.Register("sussex_ac", "萨塞克斯大学腐败问题研究所",
	...
	// 访问下一页 Index ***
	w.OnHTML(`[class="paginationControl bottom clear"] span[name="next"] > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(ctx.Url + element.Attr("href"), Crawler.Index)
		})

	// 访问 News 从 Index ***
	w.OnHTML(`div[class="row gutter-small row-wrap"] > div > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.News)
		})

	// 访问 Expert 从 Index ***
	w.OnHTML(`[class="small-12 large-9 large-push-3 columns wysiwyg content-region"] div.content > p:nth-child(4) > a`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			w.Visit(element.Attr("href"), Crawler.Expert)
		})

	// 获取 Title ***
	w.OnHTML(`h2.lineless`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Title = strings.TrimSpace(element.Text)
		})

	// 获取 Expert's Title ***
	w.OnHTML(`[class="userInfo__userInfoGroup___Wv_dB"] div > div`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Title = strings.TrimSpace(element.Text)
		})

	// 获取 Email ***
	w.OnHTML(`.oneLineText__oneLineText____Igu4`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Email = strings.TrimSpace(element.Text)
		})

	// 获取 Expert's Description ***
	w.OnHTML(`.whiteBox__body___nZQwU > p`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Expert's Tags ***
	w.OnHTML(`.tag__body___B87S8 > .tag__value___ePSCW`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Tags = append(ctx.Tags, strings.TrimSpace(element.Text))
		})

	// 获取 TwitterId ***
	w.OnHTML(`[aria-label="Twitter profile"]`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Name ***
	w.OnHTML(`.hero__header___xfv2U`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.Name = strings.TrimSpace(element.Text)
		})

	// 获取 PublicationTime ***
	w.OnHTML(`#main > .row > div > p:nth-child(4)`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 CategoryText ***
	w.OnHTML(`#main .row h1`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
			ctx.CategoryText = strings.TrimSpace(element.Text)
		})

	// 获取 Authors ***
	w.OnHTML(`#main > .row > div > p:nth-child(3)`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	// 获取 Content ***
	w.OnHTML(`.social-wrapper > #news-item`,
		func(element *colly.HTMLElement, ctx *Crawler.Context) {
