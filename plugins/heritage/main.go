...
func init() {
	s := megaCrawler.Register("heritage", "传统基金会", "https://www.heritage.org/").
		SetStartingUrls([]string{"https://www.heritage.org/sitemap.xml"}).
	...
	s.OnHTML(".article__body-copy", func(element *colly.HTMLElement) {
		megaCrawler.SetContent(element, element.Text)
	})

	s.OnHTML(".contributor-card__contributor", func(element *colly.HTMLElement) {
