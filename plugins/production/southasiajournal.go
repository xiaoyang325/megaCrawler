...
func init() {
	w := Crawler.Register("southasiajournal", "南亚分析集团", "http://southasiajournal.net")
	...
	w.OnHTML("div[class=\"page-nav td-pb-padding-side\"]", func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	w.OnHTML("div[class=\"td-block-span6\"] h3 > a", func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	w.OnHTML("h1[class=\"entry-title\"]", func(element *colly.HTMLElement, ctx *Crawler.Context) {
		ctx.Title = element.Text
	})

	w.OnHTML(".td-post-date", func(element *colly.HTMLElement, ctx *Crawler.Context) {
	...
	w.OnHTML("div[class=\"td-post-author-name\"] > a", func(element *colly.HTMLElement, ctx *Crawler.Context) {
		// 不在Index中添加。
		if ctx.PageType != Crawler.Index {
			ctx.Authors = append(ctx.Authors, element.Text)
		}
	})

	// 从网页（/events, /e-saj-features, /blog, /reviews中的）获取分类（category）到ctx中。
	w.OnHTML("li[class=\"entry-category\"] > a", func(element *colly.HTMLElement, ctx *Crawler.Context) {
		// 若分类为空则直接添加。
		if ctx.CategoryText == "" {
	...
	w.OnHTML("div[class=\"td-ss-main-content\"] > article > div[class=\"td-post-content tagdiv-type\"]", func(element *colly.HTMLElement, ctx *Crawler.Context) {
