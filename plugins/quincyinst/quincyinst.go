...
func init() {
   w := megaCrawler.Register("Quincy Institute for Responsible Statecraft", "昆西负责任治国方略研究所",
   ...
   // 从Index中访问文章。
   w.OnHTML(".link>.post__title>a", func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
   ...
   // 从翻页器获取下一页Index并访问。
   w.OnHTML("a[class=\"next page-numbers\"]", func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
      w.Visit(element.Attr("href"), megaCrawler.Index)
   })

   // 从文章中获取Title并添加到ctx。（Report）
   w.OnHTML("h1.post__title", func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
      ctx.Title = strings.TrimSpace(element.Text)
   })

   // 从文章中获取Title并添加到ctx。（News）
   w.OnHTML("header.post__header>div.post__header--container>div.post__title",
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         ctx.Title = strings.TrimSpace(element.Text)
      })

   // 从文章中获取Title并添加到ctx。（/press/）
   w.OnHTML(".press-release-template>.press-release-header>div>.press__heading",
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         ctx.Title = strings.TrimSpace(element.Text)
      })

   // 从文章中获取Title并添加到ctx。（/event/）
   w.OnHTML(".event-header>header>.event-heading",
      func(element *colly.HTMLElement, ctx *megaCrawler.Context) {
         ctx.Title = strings.TrimSpace(element.Text)
      })
